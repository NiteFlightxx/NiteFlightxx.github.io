  ...
  return (
    ...
    >
      ...
      <section className="relative min-h-[75svh] md:min-h-[85vh] flex items-center justify-center pt-6 md:pt-12 select-none">
        ...
        <div className="w-full max-w-5xl mx-auto px-6 text-center space-y-8 relative z-10">
          ...
          <div className="space-y-4">
            ...
            <motion.p
              variants={itemVariants}
              className="font-sans font-bold text-lg md:text-2xl text-accent-primary-hover tracking-widest uppercase leading-normal"
            >
              {t.title}
            </motion.p>
          </div>
          ...
          <motion.p
            variants={itemVariants}
            className="font-sans text-2xl md:text-4xl text-text-primary max-w-4xl mx-auto font-medium leading-relaxed tracking-wide"
          >
            {t.statement}
          </motion.p>
      ...
      <section className="px-6 md:px-12 max-w-7xl mx-auto space-y-8">
        <motion.div variants={itemVariants} className="flex items-end justify-between border-b border-border-subtle pb-4">
          <div className="space-y-1">
            <h2 className="font-display font-black text-2xl md:text-5xl text-text-primary tracking-tight">{t.featuredSystems}</h2>
          </div>
          <button
            type="button"
            onClick={() => setActiveTab("projects")}
                ...
                <div className="flex flex-col h-full justify-between">
                  <div className="p-6 md:p-8 space-y-4 flex-1 flex flex-col justify-between">
                    <div className="space-y-4">
                      ...
                      <p className="text-sm text-text-secondary leading-relaxed font-sans line-clamp-3">{proj.overview}</p>
                    </div>
                    <div className="flex flex-wrap gap-1.5 pt-4 border-t border-border-subtle mt-auto">
      ...
      <section className="px-6 md:px-12 max-w-7xl mx-auto space-y-8">
        <motion.div variants={itemVariants} className="flex items-end justify-between border-b border-border-subtle pb-4">
          <div className="space-y-1">
            <h2 className="font-display font-black text-2xl md:text-5xl text-text-primary tracking-tight">{t.recentKnowledge}</h2>
          </div>
          <button
            type="button"
          ...
          {recentKnowledge.map((art) => (
            ...
            >
              <a href={`${BASE_URL}knowledge/${art.slug}`} className="block p-6 flex flex-col justify-between gap-4">
                ...
                <h3 className="font-display font-bold text-lg md:text-xl text-text-primary group-hover:text-accent-primary transition-colors">
                  {art.title}
                </h3>
                {art.excerpt && (
                  <p className="text-sm text-text-secondary leading-relaxed font-sans line-clamp-2">{art.excerpt}</p>
                )}
                {art.readTime && (
                  <div className="font-mono text-[9px] text-text-faint pt-2 border-t border-border-subtle">
                    {art.readTime}
                  </div>
                )}
              </a>
