...
tags: ["数学", "线性代数", "雅可比", "机器人", "游戏物理", "飞控"]
readTime: "阅读约40分钟"
---

> 本文综合两份原始学习笔记整理而成，并补齐了它们缺失的关键内容。原始笔记用"搭积木"的方式从一维导数逐步构建到雅可比矩阵，覆盖了概念建立、几何意义与三类工程应用（机器人、飞控、游戏物理）。但两份笔记存在两个问题：一是数学公式格式损坏严重（大量反斜杠丢失，无法渲染）；二是止步于"雅可比矩阵是什么"，没有讲透**怎么用它**——尤其是逆运动学求解、物理引擎约束求解器这两块对游戏开发至关重要的算法核心。
>
> 本文的目标：先修好所有公式、把积木重新搭稳，再补上**逆问题求解（伪逆、阻尼最小二乘、雅可比转置法）、约束求解器的有效质量矩阵、SVD 与可操作度、反向传播的链式法则**这些真正让雅可比矩阵"动起来"的知识。读完后，你应当能理解为什么机器人学、飞控、游戏物理引擎、神经网络反向传播——这四个看似无关的领域——共享着同一个数学结构。
