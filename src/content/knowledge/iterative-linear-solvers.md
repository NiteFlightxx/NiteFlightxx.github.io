...
| **并行性（原始形式）** | ✅ 天然完全并行 | ⚠️ 有数据依赖，需改造才能并行（见 §5.4） |
| **收敛速度** | 较慢 | 通常快 2 倍 |
| **GPU 适用性** | ✅ 很好 | ⚠️ 经着色/分块后可行（见 §5.4） |
| **单核 CPU** | 中等 | ✅ 更快 |
...
| 方法 | 迭代次数 | 每次迭代 | 总时间 |
|:---|:---:|:---:|:---:|
| 雅可比（并行） | 800 | 0.15 ms（8× 加速） | 120 ms |
| 高斯-赛德尔（着色并行） | ~500 | 0.2 ms | ~100 ms |
| 高斯-赛德尔（原始串行形式） | 450 | 1 ms | 450 ms |

结论：单核选高斯-赛德尔；在多核/GPU 上，高斯-赛德尔经着色后可与雅可比媲美甚至更快（迭代次数更少），雅可比的优势只是实现最简单、不用预先分析依赖。下文详细解释 GS 到底能不能并行、怎么并行。

### 5.3 何时选择哪个

- **选雅可比**：GPU 计算（流体、布料的 GPU 实现）、大规模粒子系统、SIMD 向量化——天然并行、实现最简单，不用预先分析变量间的依赖关系。
- **选高斯-赛德尔**：单核 CPU（移动设备、嵌入式）、内存受限（省一半内存）、实时模拟（更少迭代次数）、PBD 约束求解；多核上经着色后同样可并行（见 §5.4）。

### 5.4 高斯-赛德尔到底能不能并行？

很多教材一句话概括"Gauss-Seidel 不能并行"，这其实不准确。准确的说法是：**Gauss-Seidel 的原始形式有数据依赖、不能直接并行，但绝大多数实际问题的稀疏结构允许把它改造成可并行**。下面把这件事彻底讲清楚。

#### 5.4.1 为什么"看起来"不能并行：数据依赖链
...
$x_i^{(k+1)}$ 用到了"这一轮刚算出的"$x_j^{(k+1)}\,(j<i)$。于是 $x_2$ 依赖 $x_1$，$x_3$ 依赖 $x_2$，……形成一条依赖链。如果让 $x_i$ 和 $x_{i+1}$ 同时计算，$x_{i+1}$ 可能在 $x_i$ 写回之前就读了它——读到的还是旧值 $x_i^{(k)}$，于是这一步实际退化成了雅可比更新，丢失了 GS"立即用新值"的好处。

这就是"看起来不能并行"的本质：一个**写后读（RAW）冒险**，更新顺序被锁死成串行。

#### 5.4.2 关键洞察：依赖只来自非零元

但仔细看公式——$x_i$ 真的需要"刚算出的"$x_j$ 吗？只有当 $a_{ij}\neq 0$ 时，$x_j$ 才真正出现在 $x_i$ 的更新式里。如果 $a_{ij}=0$（且 $a_{ji}=0$），那么 $x_i$ 和 $x_j$ **互不影响**，同时更新它们，谁也读不到对方的值，谁也不会用到对方——既不破坏 GS 性质，也没有冒险。

所以 GS 能否并行，取决于系数矩阵的**稀疏结构**，而不是 GS 算法本身。

#### 5.4.3 依赖图与图着色

把变量画成一张图：节点 $i$ 代表 $x_i$，当 $a_{ij}\neq 0$ **或** $a_{ji}\neq 0$（两个变量有耦合）时连一条边。这是一张**依赖图**。

> **规则**：图中没有边相连的两个变量，可以同时更新而不破坏 Gauss-Seidel 语义。

把这张图做**着色**——相邻节点不同色，那么同一种颜色的所有节点互不相邻、构成一个独立集，可以**一次性全部并行更新**。一轮迭代就变成"按颜色顺序，每种颜色做一次并行 sweep"：

```
对每种颜色 c（共 χ 种）：
    并行更新所有颜色为 c 的节点   // 它们互不依赖，可同时算
```

颜色数 $\chi$ 就是每轮需要的串行 sweep 次数。$\chi$ 越小并行度越高。

#### 5.4.4 什么时候真的不能并行：稠密矩阵

当矩阵**稠密**（几乎所有 $a_{ij}\neq 0$）时，依赖图接近**完全图**——任意两个变量都耦合，任意两个节点都有边相连。这时着色需要 $n$ 种颜色，每种颜色只有一个节点，并行度退化为 1，GS 真的退回纯串行。

这才是"Gauss-Seidel 不能并行"真正成立的场景。但实际工程里稠密 GS 极少出现（稠密系统一般直接用 LU 分解或 CG），所以这个限制很少真正碰到。**物理问题的矩阵几乎都是稀疏的**——每个变量只与少数邻居耦合——依赖图远不是完全图，着色后能获得很高的并行度。

#### 5.4.5 Red-Black 染色（2 色）

最经典的特例：规则网格上的 5 点模板（2D 拉普拉斯）。每个节点只依赖上下左右 4 个邻居，依赖图是**二部图**，只需 2 种颜色——红黑染色：
...
红节点的邻居全是黑节点，黑节点的邻居全是红节点。先并行更新所有红节点（它们互不相邻、互不依赖），再并行更新所有黑节点。一轮迭代 = 2 次并行 sweep。这既有 GS 的快速收敛，又能并行——是 GPU 上求解泊松类问题的常用折中。

```cpp
// RedBlackGS.cpp — 红黑染色的并行高斯-赛德尔
...
- 3D 7 点模板：通常 2 色仍可（仍是二部图）。
- 非结构化网格：需要 $\chi$ 种颜色，$\chi$ 由图的色数决定（可能 4、8、甚至更多）。

颜色多 = 每轮串行 sweep 多 = 并行度下降，但只要 $\chi \ll n$，仍然远快于纯串行。着色是一次性预计算，成本在多轮迭代中摊销。

#### 5.4.7 域分解（Domain Decomposition）

把变量按空间分成若干块，每块交给一个线程/进程：

- **块内**：用串行 GS 顺序更新（块内依赖正常处理）。
- **块间**：各块同时计算，互不干扰。
- **边界**：块交界处的变量在别的块看来是"上一轮的旧值"——相当于块边界退化成了雅可比，块内仍是 GS。

这是一种**混合策略**：块内 GS + 块间 Jacobi。边界处的信息传播变慢，收敛速度随块数增加而下降（块越多、边界越多、信息越滞后）。改进方法：

- **重叠 Schwarz（Overlapping Schwarz）**：让块边界互相重叠一两层，缓解边界滞后。
- **加粗校正（Coarse correction）**：在粗网格上修正全局低频误差，就是多重网格的思想。

域分解是大规模并行（上千核、分布式集群）的主力方法。

#### 5.4.8 异步并行 GS（Asynchronous / Chaotic Relaxation）

去掉所有同步屏障：每个处理器独立更新自己负责的变量，读共享内存里"当前有什么值就用什么值"（可能是别的处理器上一轮、也可能是更早的旧值），不等任何人。

- **优点**：完全没有同步开销，扩展性极好，适合大规模分布式。
- **代价**：收敛性不再由 $\rho(\mathbf{B})<1$ 单独保证——还要看通信图的拓扑和延迟。理论上可能比同步版慢甚至发散，但对严格对角占优系统，实践中通常仍收敛。
- 这类方法叫**异步迭代（Chazan-Miranker, 1969）**，近年来在异步并行计算里重新受关注。

#### 5.4.9 波前并行（Wavefront）

对于带状矩阵或三角系统（如 GS 矩阵形式里解 $(\mathbf{D}+\mathbf{L})\mathbf{x}=\ldots$ 的前代换），依赖沿反对角线传播：同一反对角线上的变量互不依赖，可以并行；不同反对角线按波前依次推进。这是 GPU 上解三角系统的标准技巧。

#### 5.4.10 改变顺序会改变收敛性吗？

会，但通常影响不大。着色/分块本质是**重新排列变量的更新顺序**，而 GS 的迭代矩阵 $\mathbf{B}_{GS}=-(\mathbf{D}+\mathbf{L})^{-1}\mathbf{U}$ 依赖 $\mathbf{L},\mathbf{U}$，重排会改变它们（见 §6.3）。所以：

- 着色 GS 仍是**合法的 GS**（只是换了排序），对角占优/对称正定时仍收敛。
- 收敛率 $\rho(\mathbf{B}_{GS})$ 可能与自然排序略有不同，但量级相当。
- 域分解因边界退化为雅可比，收敛率下降更明显，需配合粗校正。

一句话总结：**Gauss-Seidel 并非"不能并行"，而是"原始形式有依赖、需先分析稀疏结构再并行"**。稀疏物理问题（网格、约束）几乎总能通过着色或分块获得高并行度；只有稠密矩阵才真正退化为串行——而那种场景本就不该用 GS。
...
SOR 继承了 GS 的数据依赖（同样可用 §5.4 的着色/分块并行化），但收敛速度通常是 GS 的 $1.5\sim2$ 倍，是单核 CPU 实时模拟的好选择。
...
|:---|:---|:---:|:---|:---:|:---:|
| **雅可比** | 任意 | 低 | 慢 | ✅ 完美 | 简单 |
| **高斯-赛德尔** | 任意 | 低 | 中等 | ⚠️ 着色后可并行 | 简单 |
| **SOR** | 任意 | 低 | 较快 | ⚠️ 着色后可并行 | 简单 |
| **CG** | 对称正定 | 低 | 快 | ⚠️ 中等 | 中等 |
...
雅可比与高斯-赛德尔的差别，归根结底是一句话：**更新一个变量时，要不要用这一轮已经算出的新值**。这个看似微小的选择带来两个后果：高斯-赛德尔收敛更快但有了数据依赖。但"有依赖"不等于"不能并行"——只要矩阵稀疏（物理问题几乎都是），就能通过依赖图着色或域分解把可并行的独立集合找出来。真正的权衡不是"可并行 vs 收敛快"，而是"天然并行无需分析（雅可比） vs 收敛快但需先分析依赖（高斯-赛德尔）"。
