---
title: "凸体碰撞检测详解 — 从 SAT 到 GJK 再到 EPA 的积木式推导"
...
GJK 在 CSO 上迭代构造最多 4 个支撑点组成的单纯形，持续寻找离原点最近的点。新支撑点无法继续推进时判定分离；原点进入四面体时判定相交并交给 EPA。

### 3.1 这一层在完整管线中的职责

本文只保留 GJK 与 SAT、Minkowski 差、EPA 的接口关系：GJK 负责“是否相交/最近距离”，EPA 负责“相交后的穿透深度与法线”。单纯形降维、四面体符号判定、收敛证明和伪代码已经在 [GJK 碰撞检测算法详解](/knowledge/gjk-collision-detection/) 中完整展开，避免在两篇文章重复维护同一套推导。

### 3.2 需要记住的工程不变量

- 单纯形始终由 CSO 支撑点构成，不显式构造 Minkowski 差。
- 支撑查询使用 $S_A(d)-S_B(-d)$，两体各自只需暴露 `Support` 接口。
- 分离、相交、退化接触分别进入 GJK 距离结果或 EPA 回退。

下面只讨论 Chaos 对这些不变量的工程化实现。
