...
```markdown
---
title: "UE FullBodyIK 插件的数学原理详解 — Humanoid IK、Goal 与 Effector 收敛迭代"
...
| 规则 | 内容 | 说明 |
|---|---|---|
| **深度标识词** | 纯数学推导→`数学原理`；含物理定律（牛顿-欧拉、空气动力学等 F=ma 类）→`数学与物理原理` | 按内容是否含物理定律二选一，是可执行规则不是主观判断 |
| **副标题** | 统一用破折号 ` — `（前后各一空格）点明价值/覆盖范围，**不用冒号** | 破折号后补充"这篇覆盖什么"，不重复 category/subtopic 已有的信息 |
| **"详解"后缀** | 一律保留 `详解` | 深度已由分类/子主题/标签体现，但"详解"统一标识本文是系统性拆解而非速记 |
| **UE 术语** | 保留英文（IK Retargeter、Control Rig、PID、RK4、XPBD 等不中文化）；UE 专属内容统一带 `UE` 前缀；纯通用数学/物理不带 | 这些是 UE 原生术语，中文化反而失真、不利搜索；"UE"前缀用于一眼区分引擎专属 vs 通用内容 |

**判定流程**：

1. 内容是否含物理定律（F=ma、运动方程、空气动力学）？
   - 是 → 深度标识词 = `数学与物理原理`
   - 否 → 深度标识词 = `数学原理`
2. 是否讲 UE 专属内容（源码、插件、引擎子系统）？
   - 是 → 主题词前加 `UE `
   - 否 → 不加前缀
3. 副标题是否补充了卡片读者还不知道的信息？
   - 是 → 加 ` — 副标题`
   - 副标题若只是复述 category/subtopic，则不必加（但默认建议加，点明覆盖范围）

**命名示例**（对照现有文章）：

| 内容 | 合规标题 |
|---|---|
| UE IK 重定向 Op 堆栈 | `UE IK Retargeter Op 堆栈的数学原理详解 — 堆栈架构与逐 Op 公式拆解` |
| 雅可比矩阵（通用数学） | `雅可比矩阵的数学原理详解 — 从导数到机器人、飞控与游戏物理的统一框架` |
| 数值积分（含 F=ma） | `物理模拟数值积分方法的数学与物理原理详解 — 从欧拉到 RK4 与 XPBD` |
| 无人机飞控（含动力学+气动） | `四轴无人机飞控的数学与物理原理详解 — 从刚体动力学到级联 PID 与混控` |
| UE 动画节点 | `UE 动画节点的数学原理详解 — IK、混合、骨骼控制与惯性化` |

**禁止的写法**：

- ❌ `XXX数学原理` 漏掉 `详解`（与其它文章不统一）
- ❌ `XXX：从…到…` 用冒号副标题（统一用破折号 ` — `）
- ❌ `虚幻引擎 XXX`（引擎称呼统一用 `UE`，不用全称）
- ❌ `XXX 统一数学语言`（"语言"不准确，它讲的是数学框架；改用"统一框架"）
- ❌ 纯术语无副标题（如 `雅可比矩阵的数学原理详解`），副标题能帮卡片读者判断是否值得点开
