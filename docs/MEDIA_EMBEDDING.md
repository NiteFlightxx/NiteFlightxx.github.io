...
| 裸 `<div>` / `<svg>` / `<canvas>` | ✅ 原样保留 | 可直接在 .md 里写图表 |
...
把 `<svg>` 作为独立块（前后空行），加 `class="inline-chart"`（已有响应式 CSS：`max-width:640px`、居中、圆角）：
...
- **参考实例**：`iterative-linear-solvers.md` §4.3 末尾的「误差随迭代衰减（对数纵坐标）」收敛曲线，是对数坐标 + 双折线 + 图例的完整范例，可照搬结构。
...
外部交互内容用 iframe，**现在就能用**，加 `class="video-embed"`（已有响应式 16:9 CSS）。

### 6.1 GeoGebra 交互画板

```html
<iframe class="video-embed" loading="lazy"
...
YouTube 等同理，用平台给的嵌入码，套上 `class="video-embed"`。
